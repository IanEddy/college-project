	...
	<div class="container  clearfix">
		<div class="one-third1 column">
			<ul>
				...
				<li class="active"><a href="students.php"><img alt="" src="img/user.png"><span>Voters</span></a></li>
		...
		<div class="two-thirds1 column">
			...
			$dsdsss='notvoted';
			$xxzz='voted';
			$sadsdsd = $db->prepare("SELECT count(*) FROM students WHERE status= :a");
				$sadsdsd->bindParam(':a', $dsdsss);
				$sadsdsd->execute();
				$rowaas = $sadsdsd->fetch(PDO::FETCH_NUM);
				$notvoted=$rowaas[0];
			$sadsdsda = $db->prepare("SELECT count(*) FROM students WHERE status= :b");
				$sadsdsda->bindParam(':b', $xxzz);
				$sadsdsda->execute();
				$rowaasa = $sadsdsda->fetch(PDO::FETCH_NUM);
				$voted=$rowaasa[0];
			$sadsdsds = $db->prepare("SELECT count(*) FROM students");
				$sadsdsds->execute();
			...
			<table id="resultTable" data-responsive="table">
				...
				<tbody>
					...
					<tr class="record">
						...
						<td><a href="#" id="<?php echo $row['id']; ?>" class="delbutton" title="Click To Delete"><img alt="" src="img/delete-item.png"></a></td>
					</tr>
					<?php
						}
					?>
