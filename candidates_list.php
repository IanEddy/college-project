...
?>
<html>
    <a href="index.php"><img alt="" src="automatedvoting/admin/img/logout.png"><span>Logout</span></a>
<head>
...
body {
	font-family: montserrat, arial, verdana;
        background:#f4f4f4 url(../automatedvoting/admin/img/bg.gif) repeat top left;
	color:#333;
}
/*form styles*/
#msform {
...
<form id="msform" name="joe" action="preview.php" method="POST" onsubmit="return sapatka()">
	...
	<fieldset>
		...
		<h3 class="fs-subtitle">Select Only One:</h3> 
		<select name="<?php echo $rowasa['name'] ?>" style="height: 51px;">
		<option value="0">Select:&nbsp;<?php echo $rowasa['name'] ?></option>
		<?php
		$dsds=$rowasa['name'];
	...
	<fieldset>
		...
		<h3 class="fs-subtitle">Must select 12: <span id="payment-total"></span></span>
		</h3>
		<?php
		$dsds=$rowasa['name'];
		$results = $db->prepare("SELECT * FROM candidates WHERE position= :a");
